...
const pendingKind = ref<"install" | "repair">("install");
...
<template>
  <section class="settings-section skill-settings" aria-labelledby="skill-settings-heading">
    <header class="skill-settings-header">
      <div class="provider-heading">
        ...
        <div>
          ...
          <p>Discover project and user skills, then install more with the Vercel skills CLI through governed approvals.</p>
        </div>
      </div>
      <osx-button v-if="!showingForm" size="small" icon="plus" :disabled="!projects.length" @click="showingForm = true">Install skills</osx-button>
    </header>
    ...
    <section v-if="pendingApproval" class="skill-approval-card" :aria-label="pendingKind === 'repair' ? 'Pending skill repair approval' : 'Pending skills install approval'">
      <header>
      ...
      <div class="skill-form-actions">
        ...
        <osx-button :disabled="activeActionId === pendingApproval.id" @click="decidePending('approve')">
          {{ pendingKind === "repair" ? "Approve repair" : "Approve install" }}
        </osx-button>
      </div>
    </section>
    ...
    <osx-empty-state
      ...
      description="Install skills from skills.sh or add SKILL.md files under .agents/skills/, .cursor/skills/, or ~/.vraxis/skills/."
      action-label="Install skills"
