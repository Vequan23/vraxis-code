...
watch(() => props.runtimes, (runtimes) => {
  ...
  selectedRuntimeId.value = runtimes.find((item) => item.id === props.settings.defaultRuntimeId)?.id
...
<template>
  <section class="settings-section harness-settings" aria-labelledby="harness-settings-heading">
    ...
    <osx-empty-state v-else title="No harnesses detected" description="Install Codex, Claude Code, Cursor Agent, or OpenCode, then check again." icon="terminal" />
  </section>
</template>
