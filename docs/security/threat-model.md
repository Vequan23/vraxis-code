...
OS-level command sandboxing, editable browser origin grants, upload policy, schema-aware secret classification beyond bounded pattern redaction, deliberate cleanup or recovery UX for preserved legacy browser profiles, disk-full recovery, centrally distributed organization trust policy, and expanded security regression coverage must be complete before a public stable release. Browser authentication state is saved after governed actions and graceful close; a process crash can lose the last asynchronous page-side state change, but cannot cause it to be written as a reusable plaintext profile. The current terminal requires an explicit high-risk approval because `shell: false` and an in-workspace cwd do not prevent an approved executable from reading other user-accessible paths. Proof trust enrollment stores only normalized Ed25519 SPKI public keys; revocation removes local trust but intentionally cannot make a previously valid signature mathematically invalid.
