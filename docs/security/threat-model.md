...
- Run agent browsing in a Vraxis-owned ephemeral Playwright context that never opens the user's personal browser profile. Persist only cookies, local storage, and IndexedDB in an AES-256-GCM envelope; keep its random key in the operating-system credential store, bind ciphertext integrity to the session identity, and use owner-only state paths where POSIX modes are available.
