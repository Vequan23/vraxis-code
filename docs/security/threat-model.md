...
- Run agent browsing in a Vraxis-owned session that never opens the user's personal browser profile. Desktop uses a sandboxed task-scoped in-memory `WebContentsView` partition; the web fallback uses an ephemeral Playwright context and may persist only cookies, local storage, and IndexedDB in an AES-256-GCM envelope. Keep the fallback key in the operating-system credential store, bind ciphertext integrity to the session identity, and use owner-only state paths where POSIX modes are available.
- Keep native browser actuation out of the product renderer. Vraxis Desktop exposes layout-only IPC and gives the owned local service a random per-launch bearer token for a loopback-only control endpoint. The service remains the sole approval and evidence authority.
- Migrate a legacy isolated profile by hydrating its retained origin through a locally fulfilled, network-inert request, sealing the extracted state, and preserving the source profile owner-only for recovery. Never transmit legacy cookies merely to migrate storage.
...
- Verify signed team-policy bundles against canonical content, artifact digest, issue and expiration dates, and the active local signer registry before owner-only persistence. Team policy can only force ask or deny, never allow. Invalid, expired, or untrusted installed policy forces fresh decisions, and explicit removal discloses that local authority may widen.
...
- Publish macOS releases only from a matching semantic-version tag after quality, browser, packaged smoke, signing, notarization, and production dependency gates. Re-hash the disk image and verify product identity, stable channel, tagged asset URL, byte length, and signed/notarized manifest flags before creating the GitHub release. Keep Apple credentials in runtime secrets and delete temporary keychain material on every outcome.

## Open work

OS-level command sandboxing, editable browser origin grants, upload policy, schema-aware secret classification beyond bounded pattern redaction, deliberate cleanup or recovery UX for preserved legacy browser profiles, disk-full recovery, administrator-locked team-policy distribution and removal, Windows/Linux signed distribution, in-app update delivery, and expanded security regression coverage must be complete before a public stable release. Browser authentication state is saved after governed actions and graceful close; a process crash can lose the last asynchronous page-side state change, but cannot cause it to be written as a reusable plaintext profile. The current terminal requires an explicit high-risk approval because `shell: false` and an in-workspace cwd do not prevent an approved executable from reading other user-accessible paths. Proof trust enrollment stores only normalized Ed25519 SPKI public keys; revocation removes local trust but intentionally cannot make a previously valid signature mathematically invalid. Portable team policy is locally enforceable and auditable, but a user who controls the installation can explicitly remove it. The macOS workflow is prepared for Apple credentials but cannot prove the real signing identity or notarization service behavior until those repository secrets are configured and a tagged release succeeds.
