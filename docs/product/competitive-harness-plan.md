...
5. Keep the shipped clean-install readiness budget, packaged smoke, unexpected-exit recovery, keyboard/current-step coverage, and fail-closed signed macOS release workflow green; finish in-app update delivery, Windows/Linux signing, broader accessibility, and privacy-preserving crash reporting before expanding Understand into persistent project memory.
