...
5. Keep the shipped clean-install readiness budget, packaged smoke, unexpected-exit recovery and user-mediated private incident handoff, keyboard/current-step coverage, automated WCAG regression, and fail-closed signed macOS release workflow green; finish native crash capture with explicit upload consent, manual assistive-technology validation, in-app update delivery, and Windows/Linux signing before expanding Understand into persistent project memory.
